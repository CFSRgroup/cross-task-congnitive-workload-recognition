...
batch_size = 200  #train_dataset里面的，批的大小
num_epochs = 10   #迭代次数
...
# 1
# viz = Visdom() #可视化loss和acc python -m visdom.server
# viz.line([[0., 0.]], [0], win='train', opts=dict(title='Train', legend=['loss', 'acc']))#可视化loss
for epoch in range(18):
    ...
    for i, (images, labels) in enumerate(train_loader):#test_loader
        ...
        outputs, attention_output = model(images)
...
with torch.no_grad():
    ...
    for images, labels in test_loader:#train_loader
        ...
        # attention = np.average(attention_output.cpu().detach().numpy(), axis=0)
        # df = pd.DataFrame(attention)
        # corr = df.corr()
...
#         print("每种情感的识别准确率为：{0}".format([rate * 100 for rate in corrects / per_kinds]))
        ...
        #                  color="black" if info > thresh else "red")
        #
        # plt.tight_layout()  # 保证图不重叠
        # plt.yticks(range(Workload), labels, fontsize=10)
        # plt.xticks(range(Workload), labels, rotation=45, fontsize=10)  # X轴字体倾斜45°
        # plt.show()
        # plt.close()
# 3
